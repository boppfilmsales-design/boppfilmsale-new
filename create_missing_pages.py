#!/usr/bin/env python3
"""
Create missing BOPP and BOPET product pages
"""
...
<a href="index.html#{category}" class="back-btn">← Back to {category_name} Products</a>
<div class="sec-label">{category_name}</div>
<h1 class="sec-title">{product_name}</h1>
<div class="sec-zh">{product_name_zh}</div>

<div class="desc-block">
<div class="desc-en">{description_en}</div>
<div class="desc-zh">{description_zh}</div>
</div>
...
<div class="app-grid">{applications}</div>
...
missing_bopp = [
    {
        "filename": "bopp-flower-mini-printed.html",
        "title": "BOPP flower wrapping film smaller roll/sheet cut size printed",
        "title_zh": "Bopp花卉包装膜小卷/片印刷",
        "description": "Printed smaller rolls or sheets for specialty flower shops from AEC GROUP.",
        "category": "bopp",
        "category_name": "BOPP Flower Wrapping Film",
        "product_name": "BOPP flower wrapping film smaller roll/sheet cut size printed",
        "product_name_zh": "Bopp花卉包装膜小卷/片印刷",
        "description_en": "BOPP flower wrapping film smaller roll/sheet cut size printed\n\nThickness: 18UM, 25UM, 30UM, 35UM, 40UM\nColor: Super clear (transparent), colored or printed customer's own private logos\nWidth: 500MM/600MM/700MM/800MM/1000MM, 1200MM\nLength: 30M, 60M, 100M, 120M, 150M, 180M, 200M, 600M, 1000M\nInner core: 1.5inches, 1.8 inches, 2 inches\nPackage: Standard export package (with cartons and pallets), 4rolls (or 6rolls)/carton",
        "description_zh": "BOPP flower wrapping film smaller roll/sheet cut size printed\n\n厚度：18微米、25微米、30微米、35微米、40微米\n颜色：超透明（透明）、彩色或印刷客户私有标志\n宽度：500毫米/600毫米/700毫米/800毫米/1000毫米、1200毫米\n长度：30米、60米、100米、120米、150米、180米、200米、600米、1000米\n纸芯：1.5英寸、1.8英寸、2英寸\n包装：标准出口包装（带纸箱和托盘），4卷（或6卷）/箱",
        "features": "<span class=\"app-pill\">Printed designs · 印刷图案</span><span class=\"app-pill\">Small format · 小规格</span><span class=\"app-pill\">Custom logos · 定制标志</span>",
        "applications": "<span class=\"app-pill\">Retail flower shops · 零售花店</span><span class=\"app-pill\">Specialty packaging · 特种包装</span>"
    },
    # Add more missing pages here...
]

# Create the pages
output_dir = r"C:\Users\DELL\Desktop\files"

for page in missing_bopp:
