...
// ── Serve static data file as JavaScript (for when DB is down) ─
function serveStaticDataAsJs(res) {
  const staticData = tryReadStaticData();
  if (staticData) {
    const js = `// Auto-generated by API (fallback) - ${new Date().toISOString()}\n` +
               `var productsData = ${JSON.stringify(staticData, null, 2)};\n`;
...
module.exports = async function handler(req, res) {
  try {
    ...
    try {
      ...
      console.error('DB connection failed, falling back to static file:', dbErr.message);
      // DB unavailable (quota exceeded etc.), serve from static file
      if (req.method === 'GET' && req.query && req.query.format === 'js') {
        return serveStaticDataAsJs(res);
      }
      if (req.method === 'GET') {
        const fallbackData = tryReadStaticData() || {};
        return jsonResponse(res, { success: true, data: fallbackData, source: 'static' });
      }
      if (req.method === 'POST') {
        return jsonResponse(res, { success: false, error: 'Database unavailable. Post to DB failed. Please try again later.' }, 503);
      }
    }
