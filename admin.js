...
function showDataIncompleteBanner() {
  ...
  banner.innerHTML = '⚠️ <strong>当前加载到的产品数量（' + APP_DATA.products.length + ' 个）明显少于本机历史记录（' + lastGood + ' 个）。</strong>' +
    ...
    '<button onclick="document.getElementById(\'aecDataIncompleteBanner\').remove()" style="margin-left:12px;background:transparent;border:1px solid #ff5252;color:#ffdcdc;border-radius:4px;padding:2px 10px;cursor:pointer">我知道了</button>';
...
function loadData() {
  ...
  if (sourceProducts.length > 0) {
    sourceProducts.forEach(function(p) {
      if (!savedIds[p.id] && !deletedIds[p.id]) {
        mergedProducts.push(p);
      }
    });
  }
...
function trySaveImageToDisk(file) {
  ...
  window._siteFolderHandle.getDirectoryHandle('images', { create: true }).then(function(imagesDir) {
    ...
    showToast('图片已保存到 images/' + file.name + ' ✅');
  }).catch(function(err) {
    console.log('自动保存图片失败，将使用本地预览:', err.message);
  });
}
